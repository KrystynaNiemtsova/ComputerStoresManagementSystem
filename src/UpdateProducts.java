...
public class UpdateProducts extends JPanel {
    ...
    public UpdateProducts (Object givenValue) {
        AtomicReference<ArrayList<Device>> devices = new AtomicReference<>(DatabaseFileReader.getAllItems());
        if (givenValue != null) {
        ...
        switch (selectedRow.getCategory()) {
            case "Desktop PC" -> {
                ArrayList<DesktopPC> desktopPC = DatabaseFileReader.getDesktopPC();
            ...
            case "Laptop" -> {
                ArrayList<Laptop> laptop = DatabaseFileReader.getLaptop();
            ...
            case "Tablet" -> {
                ArrayList<Tablet> tablets = DatabaseFileReader.getTablet();
        ...
        deleteButton.addActionListener(e -> {
            ...
            for (int i = 0; i < devices.get().size(); i++) {
                if (modelField.getText().equals(devices.get().get(i).getID())) {
                    ...
                    devices.set(DatabaseFileReader.getAllItems());
                    break;
                }
            }
            ...
            if (foundMatch) {
                ...
                switch (currentItem) {
                    case "Desktop":
                        for (int i = 0; i < DatabaseFileReader.getDesktopPC().size(); i++) {
                            if (DatabaseFileReader.getDesktopPC().get(i).getID().equals(modelField.getText())) {
                                DatabaseFileReader.getDesktopPC().remove(i);
                                break;
                            }
                        }
                    case "Laptop":
                        for (int i = 0; i < DatabaseFileReader.getLaptop().size(); i++) {
                            if (DatabaseFileReader.getLaptop().get(i).getID().equals(modelField.getText())) {
                                DatabaseFileReader.getLaptop().remove(i);
                                break;
                            }
                        }
                    case "Tablet":
                        for (int i = 0; i < DatabaseFileReader.getTablet().size(); i++) {
                            if (DatabaseFileReader.getTablet().get(i).getID().equals(modelField.getText())) {
                                DatabaseFileReader.getTablet().remove(i);
                                break;
                            }
                        }
                }
                ...
                JOptionPane.showMessageDialog(null,
                        "The record for the computer is deleted successfully.");
                productInfo.repaint();
                updateProducts.repaint();
                ComputerTable.frame.repaint();
            } else {
        ...
        addButton.addActionListener(e -> {
            ...
            if (isUnique && !anyFieldEmpty() && checkInputFloat(priceField.getText())) {
                if (Objects.equals(categoryCombo.getSelectedItem(), "Desktop PC")
                    ...
                    DatabaseFileReader.setDesktopPC(desktopPC);
                    addNewComputer();
                    devices.set(DatabaseFileReader.getAllItems());
                    ...
                    tablet.add(new Tablet((String) categoryCombo.getSelectedItem(),
                            ...
                            Float.parseFloat(priceField.getText())));
                    DatabaseFileReader.setTablet(tablet);
                    addNewComputer();
                    devices.set(DatabaseFileReader.getAllItems());
                }
                updateProducts.repaint();
                ...
                if (originalCategory.equals(newCategory)) {
                    if ("Desktop PC".equals(newCategory) && checkInputInt(memoryField.getText())
                        ...
                        for (int j = 0; j < desktopPC.size(); j++) {
                            ...
                            && checkInputInt(memoryField.getText())
                            && checkInputInt(SSDField.getText())) {
                        ArrayList<Laptop> laptops = DatabaseFileReader.getLaptop();
                        for (int j = 0; j < laptops.size(); j++) {
                            if (laptops.get(j).getID().equals(modelField.getText())) {
                                ...
                                DatabaseFileReader.setLaptop(laptops);
                                JOptionPane.showMessageDialog(null,
                                        "The record for the computer has been updated");
                                break;
                            }
                        ...
                        for (int j = 0; j < tablets.size(); j++) {
                            if (tablets.get(j).getID().equals(modelField.getText())) {
                                ...
                                DatabaseFileReader.setTablet(tablets);
                                JOptionPane.showMessageDialog(null,
                                        "The record for the computer has been updated");
                                break;
                            }
                    ...
                    DatabaseFileReader.setAllItems(devices.get());
